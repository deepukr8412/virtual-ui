...
export const syncLibraryComponents = async (req, res) => {
  try {
    const user = await User.findById(req.userId);

    if (!user || user.role !== "admin") {
      return res.status(403).json({
        message: "Only admin can sync components"
      });
    }
    ...
    if (!fs.existsSync(libPath)) {
      return res.status(404).json({ message: "Library index file not found" });
    }
    ...
    lines.forEach(line => {
      const match = line.match(/export\s+{\s*([^}]+)\s*}\s+from\s+"\.\/components\/([^"]+)"/);
      if (match) {
        const exports = match[1].split(',').map(e => e.trim());
        const componentPath = match[2];
        
        exports.forEach(exportName => {
          // Extract the component name from the path (e.g., "NotificationToast/NotificationToast.jsx" -> "NotificationToast")
          const pathParts = componentPath.split('/');
          const folderName = pathParts[0];
          const fileName = pathParts[1]?.replace('.jsx', '') || folderName;
          
          // Only add if the export name matches the folder or file name
    ...
    // Debug: log all existing components
    console.log("Existing components in DB:");
    existingComponents.forEach(c => {
      console.log(`  - ${c.name}: visibility=${c.visibility}, npmPackage=${c.npmPackage}`);
    });
    ...
    for (const comp of componentNames) {
      if (existingNames.has(comp.name)) {
        ...
        console.log(`Processing ${comp.name}: visibility=${existing.visibility}, npmPackage=${existing.npmPackage}`);
        
        if (existing.visibility !== "public" || existing.npmPackage !== "virtual-ui-lib") {
          existing.visibility = "public";
          existing.npmPackage = "virtual-ui-lib";
          await existing.save();
          synced++;
          results.push({ name: comp.name, status: "updated to public", was: existing.visibility });
        } else {
          skipped++;
          results.push({ name: comp.name, status: "already public" });
        }
        continue;
      }
      ...
      if (!fs.existsSync(componentFilePath)) {
        skipped++;
        results.push({ name: comp.name, status: "file not found" });
        continue;
      }
      ...
      synced++;
      results.push({ name: comp.name, status: "created as public" });
    }
