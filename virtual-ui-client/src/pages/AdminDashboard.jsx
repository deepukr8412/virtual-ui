...
import { setUserData, setAllComponents } from "../redux/userSlice";
...
function AddComponentForm({ onSaved }) {
  ...
  const handlePublish = async () => {
    ...
    try {
      ...
      // Refetch components to update sidebar
...
export default function AdminDashboard() {
  ...
  const handleSyncLibrary = async () => {
    setSyncing(true);
    try {
      const res = await axios.post(`${ServerUrl}/api/component/sync-library`, {}, {
        withCredentials: true
      });
      alert(`Sync complete: ${res.data.synced} synced, ${res.data.skipped} skipped`);
